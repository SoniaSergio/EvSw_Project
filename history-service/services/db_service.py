...
def _decrypt_doc(doc: dict, exclude_signal: bool = False) -> dict:
    """Decifra il documento letto da MongoDB al volo."""
    ...
    # Decifra Ground Truth (Se presente e cifrato)
    if "ground_truth" in doc and isinstance(doc["ground_truth"], str):
        try:
            ...
            # Fallback se il dato non è in formato JSON cifrato
            doc["ground_truth"] = doc["ground_truth"]
        
    # Decifra o rimuovi il segnale
    if exclude_signal:
        doc.pop("signal", None)
